...
static void lcd_init(void)
{
     ...
     */
    lcd_write_cmd(0x06);
    /* display on/off; D = 1 for display on, C = 1 for cursor on; B = 0 for
     * blinking off*/
    lcd_write_cmd(0x0E);
    // lcd_write_cmd( 0x0F );
}
 ...
 *  Suppose that you decided to segment the display into 4 segments of 8 digits
 each.
 ...
 *  NOTE: maybe, you want to display arbitrary content on a given segment. For
 example:
 ...
 * segment. For example: printAtSeg(1, "S%i: %.1f", 1, 3.14); This requires the
 * use of variadic arguments with proper string formatting. Please check the
 * url: https://en.cppreference.com/w/c/language/variadic You can redeclare
 * printAtSeg as: void printAtSeg(int seg, const char* fmt, ...);
 */
